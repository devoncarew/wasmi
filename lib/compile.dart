...
CompiledFn compileFunction(
  ...
  final module = function.module;

  Bytecode translate(Instruction instr) {
    switch (instr.opcode) {
      ...
      case Opcode.globalGet:
        final index = instr.immediate_0 as int;
        if (index < 0 || index >= module.globals.length) {
          throw FormatException('unknown global');
        }
        return Bytecode(Bytecode.globalGet, i0: index);
      case Opcode.globalSet:
        ...
        final global = module.globals[index];
        if (!global.mutable) {
          throw FormatException('global is immutable');
        }

        return Bytecode(Bytecode.globalSet, i0: index);
  ...
  for (final instruction in function.instructions) {
    ...
    if (opcode == Opcode.call) {
      final fn = module.functions[instruction.immediate_0 as int];
    ...
    final bytecode = translate(instruction);
  ...
  return CompiledFn(
      executionModule, function, functionType, bytecodes, maxHeight);
}
